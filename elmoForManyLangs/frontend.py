...
def create_one_batch(x, word2id, char2id, config, oov='<oov>', pad='<pad>', sort=False):
    """
...
def create_batches(titles, indices, x, batch_size, word2id, char2id, config, perm=None, shuffle=False, sort=False, text=None):
    """
    :param titles: titles
    :param indices: indices
    :param x: List[List[str]] test
    :param batch_size:
    :param word2id:
    :param char2id:
    :param config:
    ...
    batch_title, batch_indices, batches_w, batches_c, batches_lens, batches_masks, batches_text = [], [], [], [], [], [], []
    size = batch_size
    ...
    for i in range(nbatch):
        ...
        if text is not None:
            ...
            batch_title.append(titles[start_id:end_id])
            batch_indices.append(indices[start_id: end_id])
    ...
    if sort:
        ...
        if text is not None:
            ...
            batch_title = [batch_title[i] for i in perm]

    logging.info("{} batches, avg len: {:.1f}".format(nbatch, sum_len / len(x)))
    if text is not None:
        return batches_w, batches_c, batches_lens, batches_masks, batches_text, batch_title, batch_indices
    return batches_w, batches_c, batches_lens, batches_masks, batch_title, batch_indices
...
class Model(nn.Module):
    ...
    def forward(self, word_inp, chars_package, mask_package):
        ...
        return encoder_output  # [3, 5, 247, 1024]
