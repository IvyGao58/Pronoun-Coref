...
class KnowledgePronounCorefModel(object):
    ...
    def tensorize_pronoun_example(self, example, is_training):
        ...
        n_len = max_candidate_NP_length  # number of status
    ...
    def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, p_len,
                                 s_len, speaker_ids, is_training, gold_starts, gold_ends, number_features,
    ...
    def use_attention(self, emb_list):
        ...
        flattened_emb = tf.expand_dims(flattened_emb, 0)  # [1, k1+k2+k3, emb]

        # [1, ?, 256]
        attn_out = util.attention_layer(flattened_emb, flattened_emb, size_per_head=self.config["size_per_head"])
        return attn_out

    def use_biaffine(self, attn_out, p_len, s_len):
        ...
        biaffine = tf.squeeze(biaffine_pos, 2)  # [k+c, k+c]

        p_s_mat = tf.slice(biaffine, begin=[0, p_len], size=[p_len, s_len])  # [k, c]
        s_p_mat = tf.slice(biaffine, begin=[p_len, 0], size=[s_len, p_len])  # [c, k]
